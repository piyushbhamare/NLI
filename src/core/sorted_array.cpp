...
SortedArray::SortedArray(std::vector<KeyValuePair> data) : data_(std::move(data)) {
...
SearchResult SortedArray::lookup(Key key) const {
    ...
    size_t probe_count = (size_t)std::log2(data_.size()) + 1;
...
bool SortedArray::insert(const KeyValuePair& kv) {
    ...
    if (it != data_.end() && it->key == kv.key) { it->value = kv.value; return false; }
    data_.insert(it, kv);
    return true;
}
bool SortedArray::remove(Key key) {
    auto it = std::lower_bound(data_.begin(), data_.end(), KeyValuePair(key, 0));
    if (it != data_.end() && it->key == key) { data_.erase(it); return true; }
    return false;
}
